...
<div class="wangid_conbox">
    ...
    <table class="layui-table" lay-filter="mylist" lay-size="lg">
        ...
        <tr>
            ...
            <th lay-data="{field:'id', align:'center',width:50}">id</th>
            <th lay-data="{field:'teacher_no',align:'center', minWidth:50}">教师工号</th>
            <th lay-data="{field:'teacher_name',align:'center',minWidth:60}">姓名</th>
            <th lay-data="{field:'teacher_pwd',align:'center',minWidth:30}">密码</th>
            <th lay-data="{field:'sex',align:'center',width:80}">性别</th>
            <th lay-data="{field:'tel',align:'center',width:130}">电话</th>
            <th lay-data="{field:'dep_id',align:'center',minWidth:50}">系别名称</th>
...
<script type="text/javascript">
	function update(id){
		layer.open({
			...
			// time: 2000, //2秒后自动关闭
			anim: 5,//动画
			content: ['teacher_update?id='+id, 'no'], //iframe的url，no代表不显示滚动条 
			});
	}
	...
	function del(id){
		...
		layer.confirm('你确定要删除该信息吗？', {
		  btn: ['确定','取消'] //按钮
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/delete_teacher?id="+id,
				async:false,
				type:"post",
				dataType:"json",
