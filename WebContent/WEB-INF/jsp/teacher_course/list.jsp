    ...
    <table class="layui-table" lay-filter="mylist" lay-size="lg">
        <thead>
        	<tr>
            	...
            	<th lay-data="{field:'term',align:'center',minWidth:130}">学期</th>  
            	<th lay-data="{field:'time',align:'center', minWidth:130}">教师</th>
            	<th lay-data="{field:'classes',align:'center',minWidth:130}">班级</th> 
            	<th lay-data="{field:'course',align:'center',minWidth:130}">课程</th>        
            	<th lay-data="{field:'option',align:'center',width:200,fixed: 'right'}">操作</th>
        	</tr>
        </thead>
        <tbody>
        <c:forEach items="${teacher_course}" var="item">
        	<tr>
            	<td></td>
            	<td>${item.id}</td>
            	<td>${item.term.term_name}</td>
...
<script type="text/javascript">
   ...
   function update(id){	
	   layer.open({
           ...
           // time: 2000, //2秒后自动关闭
           anim: 5,//动画
           content: ['teacher_course_update?id='+id, 'no'], //iframe的url，no代表不显示滚动条
       });
    }
	...
	function del(id){
		...
		layer.confirm('你确定要删除该信息吗？', {
		  btn: ['确定','取消'] //按钮
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/delete_teacher_course?id="+id,
				async:false,
				type:"post",				
				dataType:"json",
            ...
            switch(obj.event){
                case 'add':
                    ...
                    layer.open({
                        ...
                        // time: 2000, //2秒后自动关闭
                        anim: 5,//动画
                        content: ['teacher_course_add', 'no'], //iframe的url，no代表不显示滚动条
                    });
                    //
                    break;
                case 'update':
